...
parser.add_argument('-s', '--max_seq', default=500, type=int, help='max sentence length')
...
parser.add_argument('-t', '--shards', default=1, type=int, help='export data to this many different shards')
...
parser.add_argument('-m', '--max_distance', default=-1, type=int, help='throw out examples with mentions too far apart')
...
in_f = gzip.open(args.input_file, 'rt', encoding='utf8') if args.input_file.endswith('gz') else codecs.open(args.input_file, 'r', args.encoding)
    ...
    try:
        ...
        try:
            ...
            if not pubmed_filter or pub_id in pubmed_filter:
                ...
                replaced_text = [] # replace entity in `current_pub` with dummy_token
                last = 0
                annotation_map = {}
        ...
        in_f.close()
        ner_out_file.close()
        for shard_id in range(0, int(args.shards)):
